...
func publicize(ctx context.Context, items []appdb.Restaurant) []appdb.Restaurant {
	...
	if demovis.IsRestrictedGuest(ctx) {
		kept := make([]appdb.Restaurant, 0, len(items))
...
func ListNearby(ctx context.Context, req *ListNearbyRequest) (*ListResponse, error) {
	...
	for _, r := range all {
		...
		if r.WorksFromClientAddress {
			if wantPostal != "" && strings.ToLower(strings.TrimSpace(r.PostalCode)) == wantPostal {
				out = append(out, r)
			}
			continue
		}
		...
		} else if wantPostal != "" && strings.ToLower(strings.TrimSpace(r.PostalCode)) == wantPostal {
...
func Create(ctx context.Context, req *CreateRequest) (*appdb.Restaurant, error) {
	...
	// Booking partner: Tier 2, shown to Both, R300/month base. The accounting
	...
	if req.OffersBookings {
		// Table-booking partner: shown to Both, R300 Booking plan.
		req.AccessLevel = "Booking"
		req.GuestType = "Both"
	} else if len(req.PreOrderItems) > 0 {
		// Pre-orders bill on the R300 Booking plan too, but the restaurant stays
...
func Update(ctx context.Context, req *UpdateRequest) (*appdb.Restaurant, error) {
	...
	if req.OffersBookings != nil && *req.OffersBookings {
		// Table-booking partner: shown to Both on the R300 Booking plan.
		booking := "Booking"
		both := "Both"
		req.AccessLevel = &booking
		req.GuestType = &both
	} else if len(req.PreOrderItems) > 0 {
		// Pre-orders bill on the R300 Booking plan but keep the chosen audience
	...
	// Keep the billing subscription in sync with the edited profile. In
	// particular this propagates a changed official rep code to the
	// subscription's rep_code so activation bills (or complimentary-exempts)
	// against the CURRENT rep, not the one captured at onboarding. Idempotent
	// and must not block the edit — it never touches next_bill_date or status,
	// so a paused/unactivated subscription stays paused.
	if subErr := billing.EnsureSubscription(ctx, "restaurant", r.ID, r.AccessLevel, r.GuestType, r.OfficialRepCode); subErr != nil {
		log.Printf("restaurant %d updated but subscription sync failed: %v", r.ID, subErr)
	}
	return r, nil
}
