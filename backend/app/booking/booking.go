...
// partnerInfo pulls the fields the booking flow needs from whichever entity
// the booking is for: the display name, whether it's a Booking partner
// (accessLevel == "Booking"), and its authoritative bookable items — which we
// use to price the booking server-side rather than trusting the client.
...
func Create(ctx context.Context, req *CreateRequest) (*appdb.Booking, error) {
	...
	// Re-price from the partner's own items, matched by (case-insensitive) name,
