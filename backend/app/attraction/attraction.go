...
func ListNearby(ctx context.Context, req *ListNearbyRequest) (*ListResponse, error) {
	...
	for _, a := range all {
		...
		if a.WorksFromClientAddress {
			if wantPostal != "" && strings.ToLower(strings.TrimSpace(a.PostalCode)) == wantPostal {
				out = append(out, a)
			}
			continue
		}
		...
		} else if wantPostal != "" && strings.ToLower(strings.TrimSpace(a.PostalCode)) == wantPostal {
...
func Create(ctx context.Context, req *CreateRequest) (*appdb.AttractionData, error) {
	...
	dedupe.CheckOnCreate(ctx, "attractions", "contact_number", "attraction", created.ID,
...
func Update(ctx context.Context, req *UpdateRequest) (*appdb.AttractionData, error) {
	...
	if !auth.IsPrivileged(ctx) {
		...
		if d == nil || d.User == nil || d.User.Role != "Partner" || d.User.EntityType != "attraction" || d.User.EntityID != id {
			return nil, &errs.Error{Code: errs.PermissionDenied, Message: "you can only edit your own profile"}
		}
		storedEditCode, ecErr := attractions.GetEditCode(ctx, id)
		if ecErr != nil {
	...
	if subErr := billing.EnsureSubscription(ctx, "attraction", item.ID, item.AccessLevel, item.GuestType, item.OfficialRepCode); subErr != nil {
		log.Printf("attraction %d updated but subscription sync failed: %v", item.ID, subErr)
	}
	return item, nil
}
...
func ExportAttractions(ctx context.Context) (*CSVResponse, error) {
	...
	for _, a := range items {
		_ = w.Write([]string{
			...
			strconv.FormatBool(a.OffersBookings),
		})
	}
	w.Flush()
	return &CSVResponse{CSV: sb.String()}, nil
