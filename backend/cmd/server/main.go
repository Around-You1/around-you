...
func main() {
	...
	r.auth("POST /auth/rep/id-document", httpx.Body(auth.SetRepIDDocument))
	...
	r.auth("POST /auth/create-accountant", httpx.Body(auth.CreateAccountant))
	r.auth("GET /auth/accountants", httpx.Empty(auth.ListAccountants))
	...
	// Estate CSV import/export (agencies, agents, properties)
	r.auth("GET /estate/agency/template", httpx.Empty(estate.AgencyTemplate))
	r.auth("GET /estate/agency/export", httpx.Empty(estate.ExportAgencies))
	r.auth("POST /estate/agency/import", httpx.Body(estate.ImportAgencies))
	r.auth("GET /estate/agent/template", httpx.Empty(estate.AgentTemplate))
	r.auth("GET /estate/agent/export", httpx.Empty(estate.ExportAgents))
	r.auth("POST /estate/agent/import", httpx.Body(estate.ImportAgents))
	r.auth("GET /estate/property/template", httpx.Empty(estate.PropertyTemplate))
	r.auth("GET /estate/property/export", httpx.Empty(estate.ExportProperties))
	r.auth("POST /estate/property/import", httpx.Body(estate.ImportProperties))
	...
	r.auth("POST /preorder", httpx.Body(preorder.Submit))
